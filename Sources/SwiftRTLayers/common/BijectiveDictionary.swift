...
{
    public var valueForKey: [Key:Value]
    public var keyForValue: [Value:Key]
    ...
    @inlinable public init<T: Sequence>(_ mapping: T)
    where T.Element == (Key, Value) {
    ...
    @inlinable public init(_ dictionary: [Key: Value]) {
        valueForKey = dictionary
        keyForValue = .init(uniqueKeysWithValues: dictionary.lazy.map {
            ($1, $0)
        })
    }
    ...
    @inlinable public subscript(key: Key) -> Value? {
        ...
        set(value) {
            if let value = value {
                if let k_0 = self.keyForValue.updateValue(key, forKey: value),
                   k_0 != key
                {
                    self.valueForKey[k_0] = nil
                }
                if let v_0 = self.valueForKey.updateValue(value, forKey: key),
                   v_0 != value
                {
            ...
            assert(valueForKey.count == keyForValue.count,
                   "forward count: \(valueForKey.count), backward count:" +
                    " \(keyForValue.count), \(self)")
        }
    }
    ...
    @inlinable public func key(_ value: Value) -> Key? { keyForValue[value] }
}
