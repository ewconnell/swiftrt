...
extension DeviceQueue {
    ...
    @inlinable func replaceMapOp<S,E>(
        ...
        func execute<A: Collection, B: Collection, C: Collection, O: MutableCollection>(
            _ a: A, _ b: B, _ c: C, _ out: O
        ) where A.Element == B.Element, C.Element == Bool, O.Element == A.Element {
            var out = out
            if mode == .sync {
                zip(out.indices, zip(a.indices, zip(b.indices, c))).forEach {
                    out[$0] = $1.1.1 ? b[$1.1.0] : a[$1.0] 
                }
            } else {
                queue.async(group: group) {
                    zip(out.indices, zip(a.indices, zip(b.indices, c))).forEach {
        ...
        if a.isContiguous {
            if b.isContiguous {
                if c.isContiguous {
                    execute(a.buffer, b.buffer, c.buffer, out)
                } else {
                    execute(a.buffer, b.buffer, c.elements, out)
                }
            } else {
                if c.isContiguous {
                    execute(a.buffer, b.elements, c.buffer, out)
                } else {
                    execute(a.buffer, b.elements, c.elements, out)
                }
            }
        } else {
            if b.isContiguous {
                if c.isContiguous {
                    execute(a.elements, b.buffer, c.buffer, out)
                } else {
                    execute(a.elements, b.buffer, c.elements, out)
                }
            } else {
                if c.isContiguous {
                    execute(a.elements, b.elements, c.buffer, out)
                } else {
                    execute(a.elements, b.elements, c.elements, out)
                }
            }
        }
    }
