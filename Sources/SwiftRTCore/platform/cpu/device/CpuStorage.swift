...
public final class CpuStorage: StorageBuffer {
  ...
  @inlinable public init<Element>(
    ...
    #if DEBUG
      diagnostic(.alloc, "\(self.name) " + "\(Element.self)[\(count)]", categories: .dataAlloc)
    #endif
  }
  ...
  @inlinable public init<Element>(
    ...
    diagnostic(
      .reference, "\(self.name) " + "\(Element.self)[\(buffer.count)]", categories: .dataAlloc)
  }
  ...
  @inlinable public func synchronize(
    ...
    if let last = lastQueue, last.id != queue.id {
      diagnostic(
        .sync,
        "\(queue.name) will wait for" + " \(last.name) to "
