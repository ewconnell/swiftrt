...
extension StorageBuffer {
  ...
  ///  - name: the name of the tensor
  @inlinable public init<Element: StorageElement>(
    type: Element.Type,
    count: Int,
    name: String
  ) {
    self.init(
      storedType: Element.Stored.self,
  ...
  @inlinable public func countOf<Element>(type: Element.Type) -> Int {
    assert(
      byteCount % MemoryLayout<Element>.size == 0,
