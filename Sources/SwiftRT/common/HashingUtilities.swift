...
public extension FixedWidthInteger {
    ...
    func bytes(count byteCount: Int = MemoryLayout<Self>.size) -> [UInt8] {
        ...
        return withUnsafePointer(to: &littleEndianValue) {
            $0.withMemoryRebound(to: UInt8.self, capacity: actualByteCount)
            { pointer in
...
public extension Array where Element == UInt8 {
    ...
    @inlinable func sha1() -> SIMD32<UInt8> {
        ...
        // Append `0` bits until the length of `accumulated`
        // in bits is 448 (mod 512).
        ...
        // Step 2: Append the message length as a 64-bit representation
        // of `lengthInBits`.
        ...
        while index < accumulated.count {
            ...
            // in big-endian format. Extend the sixteen 32-bit words
    ...
    @inlinable func sha512() -> SIMD64<UInt8> {
        ...
        // Append one bit (`UInt8` with one bit) to the message.
        accumulated.append(0x80)
        // Append `0` bits until the length of `accumulated` in
        // bits is 448 (mod 512).
        let max = blockSize * 7 / 8
        accumulated += [UInt8](
            repeating: 0,
        ...
        while index < accumulated.count {
            ...
            // in big-endian format. Extend the sixteen 64-bit words into
            // eighty 64-bit words:
