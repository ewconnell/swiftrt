...
{
    pullback(at: x, in: fn)(R(repeating: 1, like: x))
}
...
public func gradient<T, U, R>(
    ...
    R: DifferentiableTensorView, R.Shape == T.Shape
{
    pullback(at: x, y, in: fn)(R(repeating: 1, like: x))
}
...
public func gradient<T, U, V, R>(
    ...
    R: DifferentiableTensorView, R.Shape == T.Shape
{
    pullback(at: x, y, z, in: fn)(R(repeating: 1, like: x))
}
...
extension PlatformService {
    ...
    {
        let almostEqual = elementsAlmostEqual(grad, expected,
                                              tolerance: tolerance)
            .all().element
        if !almostEqual {
            writeLog("gradient values do not match numerical jvp values")
            writeLog("gradient: \(grad.flatArray)")
            writeLog("expected: \(expected.flatArray)")
            let maxDiff = (grad - expected).absmax().element
            writeLog("maxDiff: \(maxDiff)")
        }
        return almostEqual
    }
}
