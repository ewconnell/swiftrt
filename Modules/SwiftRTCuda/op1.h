...
// `packed` is a version of the operator where types smaller than 32 bit
// are retyped into packed versions to use with gpu SIMD instructions
#define Op1_TO(OpName, name, conformance) \
template<typename _A, typename _O> struct OpName { \
    typedef _A A; typedef _O Out; \
    typedef typename packed<A>::type PA; \
    typedef typename matching_packed<PA,Out>::type PO; \
    typedef OpName<PA,PO> packed; \
...
//==============================================================================
// selectOut
// TODO: the use of canPack here isn't great. Rethink how `::packed` gets
// generated to remove all this boiler plate
...
static inline cudaError_t select(
    ...
    switch(atype) {
    case real32F:  return flattened<Op<float>>(a, out, count, stream);
    case real64F:  return flattened<Op<double>>(a, out, count, stream);
    case real32I:  return flattened<Op<int32_t>>(a, out, count, stream);

    // recast types that are smaller than 32 bit to their packed simd form
    case real16F:  return flattened<typename Op<float16>::packed>(a, out, count, stream);
    case real16BF: return flattened<typename Op<bfloat16>::packed>(a, out, count, stream);
    case real8U:   return flattened<typename Op<uint8_t>::packed>(a, out, count, stream);
    case real8I:   return flattened<typename Op<int8_t>::packed>(a, out, count, stream);
    case real16U:  return flattened<typename Op<uint16_t>::packed>(a, out, count, stream);
    case real16I:  return flattened<typename Op<int16_t>::packed>(a, out, count, stream);
    case boolean:  return flattened<typename Op<bool>::packed>(a, out, count, stream);

    case complex32F:  return flattened<Op<Complex<float>>>(a, out, count, stream);
    case complex16F:  return flattened<Op<Complex<float16>>>(a, out, count, stream);
    case complex16BF: return flattened<Op<Complex<bfloat16>>>(a, out, count, stream);
    default: return cudaErrorNotSupported;
    }
}
...
static inline cudaError_t select(
    ...
    switch(aDesc.type) {
    case real32F:  return selectIter<Op<float>>(a, aDesc, out, oDesc, stream);
    case real16F:  return selectIter<Op<half>>(a, aDesc, out, oDesc, stream);
    case real16BF: return selectIter<Op<bfloat16>>(a, aDesc, out, oDesc, stream);
    case real64F:  return selectIter<Op<double>>(a, aDesc, out, oDesc, stream);
    case real32I:  return selectIter<Op<int32_t>>(a, aDesc, out, oDesc, stream);
    case real8U:   return selectIter<Op<uint8_t>>(a, aDesc, out, oDesc, stream);
    case real8I:   return selectIter<Op<int8_t>>(a, aDesc, out, oDesc, stream);
    case real16U:  return selectIter<Op<uint16_t>>(a, aDesc, out, oDesc, stream);
    case real16I:  return selectIter<Op<int16_t>>(a, aDesc, out, oDesc, stream);
    case boolean:  return selectIter<Op<bool>>(a, aDesc, out, oDesc, stream);

    case complex16F:  return selectIter<Op<Complex<float16>>>(a, aDesc, out, oDesc, stream);
    case complex32F:  return selectIter<Op<Complex<float>>>(a, aDesc, out, oDesc, stream);
    case complex16BF: return selectIter<Op<Complex<bfloat16>>>(a, aDesc, out, oDesc, stream);
    default: return cudaErrorNotSupported;
    }
}

// input and output can be different type like casting or Complex Abs
template<template<typename A, typename O> class Op>
static inline cudaError_t select(
    const void* a, const TensorDescriptor& aDesc,
    void* out, const TensorDescriptor& oDesc,
